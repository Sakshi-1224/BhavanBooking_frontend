...
export default function ReportsView() {
  ...
  // 2. The Form Submit Handler (calls loadData and shows a success toast)
  const handleGenerateReport = (e) => {
    e.preventDefault();
    if (!fromDate || !toDate) return toast.warn('Please select both dates');
    loadData(fromDate, toDate).then(() => {
       // Only show the success toast when they manually click the button
       toast.success('Report generated successfully');
    });
  };

  // 3. The initial load effect (calls loadData silently on first render)
  useEffect(() => {
  ...
  return (
    <div className="bg-white shadow-sm rounded-lg border overflow-hidden p-6 print:p-0 print:border-none print:shadow-none">
      ...
      <div className="print:hidden">
        ...
        <form onSubmit={handleGenerateReport} className="flex flex-col sm:flex-row items-end gap-4 mb-8 bg-gray-50 p-4 rounded-lg border">
        ...
        <div id="printable-report">
          ...
          <div>
            ...
            {reportData.bookings.length === 0 ? (
              <p className="text-gray-500 italic text-center py-8 bg-gray-50 rounded-lg border border-dashed">No bookings found in this period.</p>
            ) : (
